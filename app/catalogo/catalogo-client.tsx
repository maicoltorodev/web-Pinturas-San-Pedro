...
import type { CatalogCover, CatalogData, CatalogFachadas } from "@/lib/site-data"
...
export function CatalogoClient({ catalogData }: { catalogData: CatalogData }) {
    ...
    return (
        ...
        >
            ...
            >
                <CoverSection coverData={catalogData.cover} />
                <FachadasSection fachadasData={catalogData.fachadas} onProductClick={openProduct} />
            </main>
...
function CoverSection({ coverData }: { coverData: CatalogCover }) {
    return (
        <section
            role="region"
 ...
 */
function FachadasSection({
    fachadasData,
    onProductClick,
}: {
    fachadasData: CatalogFachadas
    ...
    // Mapeo de label → producto del catálogo de productos
    const labelToProduct: Record<string, Product | undefined> = {
        A: vinilo,
        B: anti,
        C: estuco,
        D: brocha,
    ...
    const mainCards = fachadasData.mainProducts
        .map((mp) => ({ label: mp.label, product: labelToProduct[mp.label], blurb: mp.blurb }))
        .filter((c): c is { label: string; product: Product; blurb: string } => Boolean(c.product))

    const complementarios = fachadasData.complementarios
        .map((c) => ({ label: c.label, product: labelToProduct[c.label], name: c.displayName }))
        .filter((c): c is { label: string; product: Product; name: string } => Boolean(c.product))
